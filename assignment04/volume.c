...
struct Point make_point_euclid(double x, double y, double z) {
    struct Point p;
    p.tag = TPointEuclid;
    p.euclid.x = x;
    p.euclid.y = y;
...
static void volume_test() {
    ...
    check_within_d(
        volume(make_point_euclid(0.0, -2.0, 1.)), 
        0, EPSILON);
    check_within_d( 
        volume(make_point_euclid(2.0, 0.0, 2.0)), 
        0, EPSILON);
    check_within_d( 
        volume(make_point_euclid(1.0, 1.0, 3.0)), 
        3, EPSILON); // square root
}
...
double volume(struct Point p) {
    switch (p.tag) {
        case TPointEuclid:
            pi = 3.1;
			return sqrt(p.euclid.x * p.euclid.x + p.euclid.y* p.euclid.y) * pi * p.euclid.z;			// square root
